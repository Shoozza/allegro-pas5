...
INTERFACE
  ...
  FUNCTION al_register_font_loader (CONST ext: STRING; load: FONT_LOADER_FUNCTION): AL_BOOL; {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
  FUNCTION al_load_bitmap_font (CONST filename: STRING): ALLEGRO_FONTptr; {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
  FUNCTION al_create_builtin_font: ALLEGRO_FONTptr; CDECL;
    EXTERNAL ALLEGRO_FONT_LIB_NAME;
  FUNCTION al_load_font (CONST filename: STRING; size, flags: AL_INT): ALLEGRO_FONTptr; {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
  ...
  PROCEDURE al_draw_ustr (CONST font: ALLEGRO_FONTptr; color: ALLEGRO_COLOR; x, y: AL_FLOAT; flags: AL_INT; CONST ustr: ALLEGRO_USTRptr); CDECL;
    EXTERNAL ALLEGRO_FONT_LIB_NAME;
  PROCEDURE al_draw_text (CONST font: ALLEGRO_FONTptr; color: ALLEGRO_COLOR; x, y: AL_FLOAT; flags: AL_INT; CONST str: STRING); {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
  PROCEDURE al_draw_justified_text (CONST font: ALLEGRO_FONTptr; color: ALLEGRO_COLOR; x1, x2, y, diff: AL_FLOAT; flags: AL_INT; CONST str: STRING); {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
...
{ No "format" procedures.  Use Pascal's Format function defined at sysutils unit instead. }
  FUNCTION al_get_text_width (CONST font: ALLEGRO_FONTptr; CONST str: STRING): AL_INT; {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
  ...
  PROCEDURE al_get_ustr_dimensions (CONST f: ALLEGRO_FONTptr; CONST str: ALLEGRO_USTRptr; VAR bbx, bby, bbw, bbh: AL_INT); CDECL;
    EXTERNAL ALLEGRO_FONT_LIB_NAME;
  PROCEDURE al_get_text_dimensions (CONST f: ALLEGRO_FONTptr; CONST str: STRING; VAR bbx, bby, bbw, bbh: AL_INT); {$IFDEF HAS_INLINE}INLINE;{$ENDIF}
